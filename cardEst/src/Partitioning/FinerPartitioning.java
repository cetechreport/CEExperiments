...
public class FinerPartitioning {
    // label -> inDegPartitions
    private static Map<Integer, Map<Integer, Set<Edge>>> label2inDegPartitions = new HashMap<>();
    // terminal in_label -> inDegPartitionLabels
    private static Map<Integer, Set<Integer>> label2partitionLabels = new HashMap<>();

    // in_deg -> (in_labels, the_other_labels)
    private static Map<Integer, Pair<Set<Integer>, Set<Integer>>> inDeg2labels = new HashMap<>();
    private static Map<Integer, Set<Edge>> inDeg2buckets = new HashMap<>();

    private static int[] getIncLabels(String partitionFileName) {
        String[] nameArray = partitionFileName.split("_");
        int[] inLabels = new int[nameArray.length];
        for (int i = 0; i < nameArray.length; ++i) {
            inLabels[i] = Integer.parseInt(nameArray[i]);
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        int[] tripleList;
        BufferedReader csvReader = new BufferedReader(new FileReader(partitionFile));
        ...
        for (int label: getIncLabels(partitionFile)) {
            System.out.print(".");
