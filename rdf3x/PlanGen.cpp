...
//---------------------------------------------------------------------------
PlanGen::PlanGen(Method method) : method(method) {}
//---------------------------------------------------------------------------
PlanGen::~PlanGen()
   // Destructor
{
   ...
   // Generate base table accesses
{
   ...
   result->cardinality = getCard(result->relations, query);
...
//---------------------------------------------------------------------------
void split(string s, const string delimiter, vector<string> &splitted) {
   size_t pos = 0;
   string token;
...
//---------------------------------------------------------------------------
double findEntry(string s) {
   vector<string> splitted;
   ifstream mt;
   mt.open("catalogue.txt");
   string entry;
   while(getline(mt, entry)) {
      split(entry, ",", splitted);
      string pattern = splitted[1] + ',' + splitted[2];
      if(pattern == s) return stod(splitted[3]);
      splitted.clear();
   }
   cerr << "cannot find entry " << s << endl;
   exit(1);
}
//---------------------------------------------------------------------------
double estTrig(string s) {
   vector<string> splitted, edgeLabs, edges;
   string entry;
   split(entry, ",", splitted);
   split(splitted[1], "->", edgeLabs);
   if (edgeLabs.size() != 3) return findEntry(s);
   split(splitted[0], ";", edges);
   set<string> vertices;
   for (int i = 0;  i < edges.size(); ++i) {
      vector<string> edge;
      split(edges[i], "-", edge);
      string v1 = edge[0], v2 = edge[1];
      vertices.insert(v1);
      vertices.insert(v2);
   }
   if (vertices.size() != 3) return findEntry(s);
   ...
   while(getline(mt, entry)) {
      split(entry, ",", splitted);
      string pattern = splitted[1] + ',' + splitted[2];
      string p1 = edges[0] + ";" + edges[1] + "," + edgeLabs[0] + "->" + edgeLabs[1];
      string p2 = edges[0] + ";" + edges[2] + "," + edgeLabs[0] + "->" + edgeLabs[2];
      string p3 = edges[1] + ";" + edges[2] + "," + edgeLabs[1] + "->" + edgeLabs[2];
      if(pattern + "***" == s) ext = stod(splitted[3]);
      else if ((p1 == pattern || p2 == pattern || p3 == pattern) && min > stod(splitted[3])) min = stod(splitted[3]);
      splitted.clear();
   }
   return min * ext;
...
//---------------------------------------------------------------------------
double PlanGen::getEstimation(int count, string s, string heuristic) {
   int base = 2;
   if (count <= 3) return findEntry(s);
   int i = 7;
   if (method == Method::baselineMinMin) i = 3;
   
   int exitStatus;
   string command;
   if (count <= 10) command = "./run_opt false " + heuristic;
   else command = "./run_opt true " + heuristic;
   ...
   ifstream cardfile;
   vector<string> splitted;
   cardfile.open("estimation.csv");
   getline(cardfile, s);
   split(s, ",", splitted);
   // cout << stod(splitted[base + getIndex[method]]) << endl;
   return stod(splitted[base + i]);
}
//---------------------------------------------------------------------------
double PlanGen::getCard(const BitSet& bitset, const QueryGraph::SubQuery& query) {
   if (method == Method::rdf3x) return 1;

   vector<unsigned> edges;
   ifstream edgeFile;
   edgeFile.open("edges.txt");
   unsigned label;
   while(edgeFile >> label) {
   ...
   for (unsigned int i = 0; i < query.nodes.size(); ++i) {
      if (bitset.test(i)) {
         s1 += to_string(query.nodes.at(i).subject) + "-" + to_string(query.nodes.at(i).object) + ";";
         s2 += to_string(edges.at(i)) + "->";
         count++;
      }
   }
   s =  s1.substr(0, s1.length() - 1) + ',' + s2.substr(0, s2.length() - 2);
   output << s << endl;
   ...
   switch (method) {
      case Method::worst:
      case Method::trueCard:
         system("./run_opt trueCard");
         
         cardfile.open("card.txt");
         for(int i = 0; getline(cardfile, s); ++i) {
            if (i == 5) {
               pos = s.find_last_of(' ');
               break;
            }
         ...
         cardfile.close();
         output.close();
         cout << s.substr(pos + 1, s.length() - 1 - pos) << endl;
         return stod(s.substr(pos + 1, s.length() - 1 - pos));

      case Method::baselineMinMin:
         return getEstimation(count, s, "acyclic");

      case Method::midEdgeMaxMax:
         return getEstimation(count, s, "midEdgeClosing");
      
      case Method::trigMaxMax:
         return getEstimation(count, s, "triangleClosing");
      
      case Method::trigExtMaxMax:
         return getEstimation(count, s, "avgSampledExtensionRate");
      
      case Method::extMaxMax:
         return getEstimation(count, s, "ext");

      default:
         if (count <= 3) return findEntry(s);
         
         if (count <= 6) exitStatus = system("./run_opt false");
         else exitStatus = system("./run_opt true");
         if (exitStatus == 124) {
            cerr << "timed out" << endl;
            exit(1);
         }
         ...
         getline(cardfile, s);
         split(s, ",", splitted);
         map<PlanGen::Method, int> getIndex;
         getIndex[Method::minMin] = 3;
         getIndex[Method::allMin] = 0;
         getIndex[Method::maxMin] = 6;
         getIndex[Method::minAvg] = 5;
         getIndex[Method::allAvg] = 2;
         getIndex[Method::maxAvg] = 8;
         getIndex[Method::minMax] = 4;
         getIndex[Method::allMax] = 1;
         getIndex[Method::maxMax] = 7;
         // cout << stod(splitted[base + getIndex[method]]) << endl;
         return stod(splitted[base + getIndex[method]]);
   }
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
double PlanGen::assignCard(double problemCard, double lCard, double rCard, double selectivity) {
   double planCard;
   switch(method) {
      case Method::rdf3x:
         if ((planCard = lCard * rCard * selectivity)<1) planCard = 1;
         break;
      default:
         if ((planCard = problemCard)<1) planCard = 1;
         break;
   }
   return planCard;
}
               ...
               for (vector<JoinDescription>::const_iterator iter3=joins.begin(),limit3=joins.end();iter3!=limit3;++iter3)
                  if (((*iter3).left.subsetOf(leftRel))&&((*iter3).right.subsetOf(rightRel))) {
                     ...
                     if (lookup.count(relations)) {
                        ...
                        problem->cardinality=getCard(problem->relations, query);
                     ...
                     if (leftPlan->ordering==rightPlan->ordering) {
                        for (vector<unsigned>::const_iterator iter=joinOrderings.begin(),limit=joinOrderings.end();iter!=limit;++iter) {
                           if (leftPlan->ordering==(*iter)) {
                              ...
                              p->right=rightPlan;
			                     p->next=0;
                              p->cardinality = assignCard(problem->cardinality, leftPlan->cardinality, rightPlan->cardinality, selectivity);
