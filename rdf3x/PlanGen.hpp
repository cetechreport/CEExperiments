...
{
   public:
   enum class Method {rdf3x, worst, trueCard, minMin, allMin, maxMin, minAvg, allAvg, maxAvg, minMax, allMax, maxMax, baselineMinMin, 
	   midEdgeMaxMax, trigMaxMax, extMaxMax, trigExtMaxMax};
   ...
   ///return cardinality of query
   double getCard(const BitSet& bitset, const QueryGraph::SubQuery& query);

   double assignCard(double problemCard, double lCard, double rCard, double selectivity);
   
   double getEstimation(int count, std::string s, std::string heuristic);
